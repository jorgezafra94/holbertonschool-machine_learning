...
class Yolo:
    ...
    * anchors: the anchor boxes
    ...
    def load_images(folder_path):
        """
        * older_path: a string representing the path to the folder
    ...
    def preprocess_images(self, images):
        ...
        * Rescale all images to have pixel values in
            the range [0, 1]
        Returns a tuple of (pimages, image_shapes):
            * pimages: a numpy.ndarray of shape
              (ni, input_h, input_w, 3) containing all of the
              preprocessed images
            ...
            * image_shapes: a numpy.ndarray of shape (ni, 2)
                containing the original height and width of
                the images
    ...
    def predict(self, folder_path):
        ...
        * image_paths: a list of image paths corresponding to each prediction
            in predictions
        """
        # calling and getting images with paths
        images, image_paths = self.load_images(folder_path)

        # send the images to the preprocess
        pimages, image_shapes = self.preprocess_images(images)

        # pass the preprocess images into the model to realize prediction
        # it is gonna give us 3 outputs per image
        outputs = self.model.predict(pimages)

        predictions = []
        for i in range(pimages.shape[0]):
            # list of specified images with outputs 13, 26 and 52
            out_img = [elem[i] for elem in outputs]
            # getting the corner of anchors, confidence and scores with sigmoid
